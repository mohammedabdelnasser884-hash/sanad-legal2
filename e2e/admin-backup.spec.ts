...
test.describe.serial('النسخ الاحتياطي', () => {
  test('إنشاء نسخة احتياطية جديدة وظهورها في القائمة كأحدث نسخة', async ({ page }) => {
    ...
    //
    // 🔒 FIX (تشخيص لوجز E2E — 15 أغسطس 2026): مهلة الـ30 ثانية (اللي كانت
    // كافية وقت فيكس 1 أغسطس) بقت بتتعدى فعليًا دلوقتي — التوست بيفضل
    // بكلاس "toast" بس طول الـ30 ثانية (يعني العملية لسه شغالة ومخلصتش،
    // مش أي مشكلة في الـtoast نفسه أو في الـassertion). السبب المرجّح:
    // حجم بيانات المكتب الحقيقي اللي بيتصدّر بيكبر باستمرار مع الوقت
    // (نفس نمط تحذير فيكس 1 أغسطس، بس البيانات كبرت أكتر). تكبير المهلة
    // لـ60 ثانية تحسّبًا لتراكم إضافي مستقبلي، من غير ما نلمس منطق التصدير
    // نفسه (خارج نطاق فحص اللوجز ده — يستاهل مراجعة أداء منفصلة لو استمر
    // النمو، زي تصدير الجداول بالتوازي بدل التسلسل، موثّق في تعليق
    // useAdminBackup.ts).
    await expectToast(page, '✅ تم إنشاء النسخة الاحتياطية بنجاح', 60_000);
