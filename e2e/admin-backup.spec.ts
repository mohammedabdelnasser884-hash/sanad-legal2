...
test.describe.serial('النسخ الاحتياطي', () => {
  test('إنشاء نسخة احتياطية جديدة وظهورها في القائمة كأحدث نسخة', async ({ page }) => {
    // 🔴 CHANGED (إصلاح CI ثانٍ بعد تشغيل فعلي — 29 أغسطس 2026): مهلة الـ60
    // ثانية (اللي كانت كافية وقت فيكس 15 أغسطس، تعليق فوق) بقت هي نفسها
    // بتتعدى فعليًا دلوقتي — التشغيل الفعلي وقف بالظبط عند "Test timeout of
    // 60000ms exceeded" في نفس اللحظة اللي expectToast(..., 60_000) كان
    // مستنيها، يعني مهلة التست الكلية (60000ms الافتراضية من
    // playwright.config.ts) وصلت لأقصاها في نفس لحظة انتظار التوست بالظبط —
    // مفيش هامش خالص. نفس النمط الموثّق فوق (حجم بيانات المكتب الحقيقي
    // بيكبر باستمرار)، تكرر تاني. الحل المؤقت المتّسق مع نفس الفلسفة
    // (تكبير المهلة، من غير لمس منطق التصدير نفسه — يستاهل مراجعة أداء
    // منفصلة زي ما موثّق فوق): test.setTimeout صريح للتست ده بس (مش كل
    // الملف) + هامش حقيقي بين مهلة التوست ومهلة التست الكلية، بدل ما
    // يبقوا متطابقين بالظبط زي قبل.
    ...
    await expectToast(page, '✅ تم إنشاء النسخة الاحتياطية بنجاح', 120_000);
