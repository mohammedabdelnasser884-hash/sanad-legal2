...
function PdfViewerModal({doc, onClose}: PdfViewerModalProps){
    ...
    // 🖥️ دفعة 2.4 (تجربة الديسكتوب): على موبايل يفضل ملء الشاشة بالكامل
    // زي ما هو (fixed inset-0 بلا أي تغيير). على ديسكتوب (lg:) الخلفية
    // بتبقى overlay معتم والمحتوى بيتحط جوه بطاقة بعرض أقصى محدود
    // ومتوسّطة، بدل الامتداد لحواف الشاشة بالكامل.
    return React.createElement('div',{
        'data-testid':'pdf-viewer-modal',
        className:"fixed inset-0 z-[70] bg-black/95 lg:bg-black/80 flex flex-col lg:items-center lg:justify-center lg:p-6 fade-in",
        onClick:(e: React.MouseEvent<HTMLDivElement>) =>{if(e.target===e.currentTarget)onClose();}
    },
        React.createElement('div',{className:"flex flex-col flex-1 min-h-0 lg:flex-none lg:w-full lg:max-w-4xl lg:h-[85vh] lg:rounded-3xl lg:overflow-hidden lg:border lg:border-white/10 lg:shadow-2xl"},
            // شريط العنوان
