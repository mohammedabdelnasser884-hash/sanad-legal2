...
export interface PartyValidationError {
    ...
    partyId: string;
    field: 'name' | 'capacity' | 'address' | 'national_id' | 'legal_title';
    message: string;
 ...
 * 1. الاسم والصفة والعنوان إجباريين لكل طرف دايمًا (بغض النظر عن
 *    is_client أو client_id).
...
export function validateParties(
    ...
    for (const p of parties) {
        ...
        // بقى إجباري لكل طرف (مدعي/مدعى عليه، موكل أو خصم) في الحالتين
        // (القضية والجلسة المستقلة) — بنفس منطق فحص الاسم/الصفة فوق،
        // بغض النظر عن is_client أو client_id.
        if (!p.address.trim()) {
            errors.push({ partyId: p.id, field: 'address', message: '⚠️ عنوان الطرف مطلوب' });
        }
