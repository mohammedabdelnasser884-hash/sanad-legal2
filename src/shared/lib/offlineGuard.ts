...
// ══════════════════════════════════════════════════════════════
//  🔒 فحوصات التكرار (رقم قيد القضية / بيانات الموكل) بتتنفذ *قبل* أي
//  INSERT/UPDATE فعلي في 5 دوال حفظ.
//
//  runDuplicateCheckOfflineAware بتلف نداء فحص التكرار بنفس منطق
//  createFetchGuard: لو الفحص عدّى الـ8 ثواني (تايم آوت)، بترجع
//  skipped:true (يعني "أجّل الفحص، السيرفر هيرفض التكرار وقت الحفظ
//  الفعلي على أي حال عن طريق الـUNIQUE index") بدل ما توقف الحفظ. أي
//  خطأ حقيقي تاني (مش تايم آوت) بيتعاد رميه زي ما هو عشان المنادي يفضل
//  يعامله بنفس الأسلوب القديم (توست خطأ + إيقاف الحفظ).
//
//  ⚠️ فرع "أوف لاين من الأساس" (guard.offline) اتشال (مرحلة 6 من خطة
//  إلغاء الأوفلاين فى الكتابة — 14 سبتمبر 2026): بعد ما كل عمليات الحفظ
//  بقت بتتمنع صراحة عند navigator.onLine=false *قبل* ما توصل لفحص
//  التكرار من الأساس (المرحلة 4)، الفرع ده بقى كود ميت غير قابل
//  للوصول عمليًا — مستحيل تدخل الدالة دي وإنت أوف لاين. الإبقاء عليه
//  خطر تضليل أكتر من فايدة (بيوهم إن فيه مسار أوفلاين لسه موجود).
// ══════════════════════════════════════════════════════════════

export interface DuplicateCheckOutcome<T> {
    /** true = الفحص اتأجل (تايم آوت) — مفيش نتيجة فعلية، كمّل الحفظ عادي */
    skipped: boolean;
    result?: T;
}
