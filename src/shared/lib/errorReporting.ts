...
function getSubscriptionAwareMessage(rawError: unknown): string | null {
  ...
  // 🆕 FIX (تحقيق شامل — 9 سبتمبر 2026): مسارين لازم يتغطوا هنا مش واحد بس:
  //  1) رسالة RLS الخام من RPC/PostgREST مباشر (فيها 'tenant_write_allowed' —
  //     اسم الـpolicy نفسه) — الشرط الأصلي.
  //  2) رسالة admin-actions (Edge Function): الدالة المطابقة هناك
  //     (subscriptionAwareMessage فى supabase/functions/admin-actions/index.ts)
  //     بتترجم الرسالة الخام لنفس النص العربي الجاهز **قبل** ما ترجع للفرونت
  //     إند (عشان منسربش تفاصيل RLS التقنية عبر الشبكة). يعني الرسالة اللي
  //     توصل هنا فى المسار ده بقت مترجمة بالفعل ومفيهاش 'tenant_write_allowed'
  //     خالص — لو اكتفينا بالشرط الأصلي بس، كل عمليات admin-actions (تفعيل/
  //     تعطيل مستخدم، إنشاء محامي، تعديل بروفايل...) هتفشل تكشف القفل رغم
  //     إنها بتنادي showErrorToast صح، وهيظهر المودال العام بدل المخصص.
  //     الحل: نتحقق كمان من جزء مميز من النص المترجم نفسه (مش لازم يتطابق
  //     حرفيًا بالكامل، يكفي جزء ثابت مايتكررش فى أي رسالة تانية بالتطبيق).
  if (message.includes('tenant_write_allowed') || message.includes('وضع مشاهدة فقط دلوقتي')) {
