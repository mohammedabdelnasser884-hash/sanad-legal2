                ...
                React.createElement('div',{
                    ...
                    React.createElement('div',{className:"flex-1 min-w-0"},
                        ...
                        React.createElement('p',{className:"text-[10px] text-slate-400 mt-1 leading-relaxed"},
                            ...
                            //
                            // 🐛 FIX (٩ سبتمبر ٢٠٢٦ — بلاغ: تعذّر حفظ بوابة الموكل +
                            // "تحقق من اتصالك بالإنترنت" ظاهرين مع بعض بعد الليمت):
                            // رسائل حد الباقة/القفل (getSubscriptionAwareMessage فى
                            // errorReporting.ts — مثلاً "وصلت للحد الأقصى لحسابات
                            // بوابة الموكل... رقّي الباقة لتفعيل حسابات جديدة") مفيهاش
                            // كلمة "تحقق من" ولا "حاول" حرفيًا (فيها "رقّي"/"كلّم
                            // الإدارة" بدالهم)، فكان الشرط القديم يفشل ويضيف جملة
                            // "تحقق من اتصالك بالإنترنت" فوقها — تناقض مربك: رسالة
                            // واضحة عن حد الباقة + جملة زيادة توهم بمشكلة شبكة، مع
                            // إن العملية أصلاً مش مشكلة اتصال خالص (المودال المخصص
                            // ظهر صح فى نفس اللحظة). استبعدنا أي رسالة فيها "الباقة"
                            // أو "الاشتراك" أو "كلّم الإدارة" من إضافة الجملة دي —
                            // الرسائل دي دايمًا عندها خطوة تالية واضحة خاصة بيها.
                            (err.errorMsg
                                && !err.errorMsg.includes('تحقق من')
                                && !err.errorMsg.includes('حاول')
                                && !err.errorMsg.includes('الباقة')
                                && !err.errorMsg.includes('الاشتراك')
                                && !err.errorMsg.includes('كلّم الإدارة'))
