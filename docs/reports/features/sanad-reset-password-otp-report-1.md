...
## تحديث (فيكس رسالة خطأ مضلِّلة — نفس اليوم)
لاحظ المستخدم إنه لو حط في فورم الباسورد الجديد **نفس الباسورد
القديم بالظبط**، الشاشة كانت بتوري رسالة "تحقق من اتصال الإنترنت"
— رغم إن الإنترنت شغال 100% والمشكلة الحقيقية إن Supabase بترفض
الباسورد الجديد لو مطابق للقديم (قاعدة أمان مدمجة في GoTrue).

**السبب:** `handleSave` كان بيحوّل أي `error.message` راجع من
`db.auth.updateUser` لنفس الرسالة العامة، من غير ما يفرّق بين خطأ
شبكة فعلي وخطأ منطقي واضح.

**الفيكس:** دالة جديدة `friendlyUpdatePasswordError()` بتفحص نص
الخطأ الخام (إنجليزي، جايّ من GoTrue) وتطلّع رسالة عربية دقيقة
للحالات المعروفة:
- "نفس الباسورد القديم" → رسالة واضحة تطلب باسورد مختلف.
- "الباسورد قصير جدًا" → رسالة واضحة.
- "الجلسة منتهية/JWT" → رسالة تطلب لينك استعادة جديد.
- أي حاجة تانية غير معروفة → لسه بترجع للرسالة العامة (شبكة/سيرفر)
  زي الأول، عشان مانخترعش رسائل لأخطاء مش متوقعة.

توازن الأقواس اتفحص تاني بعد التعديل — متطابق (169/169، 76/76،
26/26).

## تحديث أمان مهم (إغلاق ثغرة تخطي شاشة الكود — نفس اليوم)
سؤال المستخدم: "هل الموضوع آمن لأي حساب يحاول إعادة التعيين؟" كشف
ثغرة حقيقية: مرحلة "تأكيد الكود" كانت بتتحكم في **واجهة الشاشة بس**
(`stage === 'password'`)، لكن `handleSave` كان لسه بينده على
`db.auth.updateUser` **مباشرة من المتصفح** — يعني أي حد معاه جلسة
الـrecovery (access token) كان يقدر يغيّر الباسورد بنداء مباشر لـ
API من غير ما يمر على خطوة الكود خالص. الحماية الفعلية كانت لسه
معتمدة على لينك الإيميل بس، مش على "تأكيدين مستقلّين" زي المفروض.

### الإصلاح
1. **إيدج فانكشن جديدة `action: set_password { password }`**
   (`password-reset-otp/index.ts`): بتاخد المسؤولية الكاملة عن تغيير
   الباسورد، وبترفض تمامًا (403) لو مفيش صف `password_reset_otps`
   لنفس المستخدم بـ`consumed_at` مُسجّل خلال آخر **10 دقايق**. لو
   الشرط اتحقق، التغيير بيتم عن طريق **Supabase Admin API**
   (`PUT /auth/v1/admin/users/:id`) بصلاحية `service_role` — مش عن
   طريق مسار GoTrue الذاتي (`/auth/v1/user`) اللي أي حد معاه access
   token عادي يقدر يناديه مباشرة. بعد النجاح، بيتم مسح صف الكود عشان
   مايتقدرش يُستخدم تاني.
2. **`ResetPasswordScreen.tsx`**: `handleSave` بقى بينده على
   `db.functions.invoke('password-reset-otp', {action:'set_password'})`
   بدل `db.auth.updateUser` مباشرة، بنفس نمط معالجة الأخطاء
   (`getEdgeFunctionErrorMessage`/`looksArabicUserMessage`) المستخدم
   في باقي مراحل الشاشة. رسالة "الباسورد الجديد لازم يكون مختلف عن
   القديم" اتنقلت لداخل الإيدج فانكشن (`mapAdminPasswordError`) بدل
   الفرونت إند، لأن الفحص بقى بيحصل هناك.

### النتيجة
دلوقتي **السيرفر نفسه** هو اللي بيفرض شرط "تأكيد الكود أولًا"، مش
الواجهة بس — حتى لو حد قدر يوصل لجلسة الـrecovery ويحاول يغيّر
الباسورد بنداء مباشر متخطّي واجهة التطبيق بالكامل، الطلب هيترفض من
غير كود متأكَّد حديث. الحماية بقت فعليًا "تأكيدين مستقلّين لملكية
الإيميل" زي ما كان مخطط له من الأول، مش تأكيد واحد بواجهة زيادة.

توازن الأقواس اتفحص في الملفين الاتنين بعد التعديل — متطابق (إيدج
فانكشن: 190/190، 107/107، 7/7 — الشاشة: 163/163، 75/75، 26/26).

⚠️ ملاحظة جانبية: فحص "الباسورد الجديد مختلف عن القديم" بقى بيحصل
عبر Admin API بدل مسار GoTrue الذاتي؛ ده تغيير في مكان الفحص بس مش
في الأمان — القيمة الأمنية الحقيقية (منع تخطي الكود) أهم بكتير من
تفصيلة الرسالة دي.

## تحديث حرج (استبدال Resend بـGmail SMTP — نفس اليوم)
سؤال المستخدم "هل الخطة اتقفلت لأي حساب؟" كشف مشكلة حقيقية: إيميل
التجربة `onboarding@resend.dev` في Resend **بيبعت بس لإيميل صاحب
حساب Resend نفسه** (قيد أمان موثّق من Resend لحماية سمعة الدومين).
يعني ميزة الكود كانت هتشتغل للمطوّر بس، ومش هتوصل لأي محامي/موظف
تاني في المكتب يحاول يستعيد باسورده — من غير ما يظهر أي خطأ واضح
للمستخدم النهائي (كانت هتترفض من Resend بـ403).

الحل الدائم المعتاد (توثيق دومين في Resend) بيحتاج المستخدم يمتلك
دومين، وهو مفيش عنده حاليًا ومش عايز حلول غير مجانية. البديل:

### الحل: Gmail SMTP بدل Resend
`sendCodeEmail()` في `password-reset-otp/index.ts` بقت بتستخدم
**denomailer** (مكتبة SMTP خفيفة لـDeno، نفس الطريقة الموثّقة رسميًا
في أمثلة Supabase لإرسال إيميلات) بدل نداء Resend API. الاتصال
بيتم عبر `smtp.gmail.com` باستخدام حساب Gmail عادي + **App
Password** (كود 16 حرف من إعدادات أمان Google، مش باسورد الحساب
الحقيقي). Gmail بيسمح بالإرسال لأي مستلم (بعكس قيد Resend
التجريبي)، وحده اليومي (~500 إيميل) كافي جدًا لمكتب صغير/متوسط —
ومجاني بالكامل، بلا حاجة لدومين.

### الـsecrets الجديدة المطلوبة (بدل RESEND_API_KEY)
- `SMTP_USER` — إيميل Gmail (نفس اللي عليه الـApp Password)
- `SMTP_PASS` — الكود المكوّن من 16 حرف
- `SMTP_HOST`/`SMTP_PORT`/`SMTP_FROM` — اختياريين، ليهم قيمة
  افتراضية تناسب Gmail مباشرة.

### ملاحظة للمستقبل
لو المكتب حصل على دومين خاص لاحقًا (زي دومين موقع Nasser Legal
Foundation)، ممكن نرجع لـResend أو أي مزوّد احترافي تاني من غير أي
تعديل في منطق التطبيق — بس هيحتاج فتح `sendCodeEmail()` من جديد
وتغيير الـsecrets، مش أي تغيير في `ResetPasswordScreen.tsx` أو
جدول قاعدة البيانات.

توازن الأقواس اتفحص بعد التعديل — متطابق في الإيدج فانكشن
(200/200، 108/108، 6/6) والشاشة (164/164، 75/75، 26/26).

## تحديث حرج تاني (استبدال Gmail SMTP بـBrevo API — نفس اليوم)
بعد ربط Gmail SMTP، جرّب المستخدم الفلو وطلع خطأ "تعذّر إرسال كود
التحقق حاليًا". السبب المرجّح (موثّق في تجارب وgithub issues
كتير خاصة بـSupabase): **Supabase Edge Functions بتمنع أو بتعلّق
الاتصالات الصادرة على SMTP ports (25/465/587)** في أغلب الحالات —
يعني الاتصال بـ`smtp.gmail.com` عبر denomailer مش موثوق فيه من
الأساس جوه بيئة Supabase، حتى لو الإعدادات (App Password، إلخ)
صح 100%.

### الحل النهائي: Brevo عبر HTTPS API
بدل أي بروتوكول SMTP خام، رجعنا لنفس أسلوب Resend الأصلي (نداء
HTTPS API عادي عن طريق `fetch` — نفس الطريقة اللي باقي الإيدج
فانكشنز في المشروع بتستخدمها بالفعل لأي خدمة خارجية، ومفيش أي قيد
عليها) لكن بمزوّد اسمه **Brevo**:
- مجاني (300 إيميل/يوم)
- **مفيش قيد "بيبعت لصاحب الحساب بس"** زي Resend
- توثيق **الإيميل نفسه بس** (كود تحقق بيتبعتلك عليه) — **مفيش أي
  دومين أو DNS مطلوب**، بعكس التوثيق الكامل اللي Resend بيطلبه
  للإرسال لمستلمين تانيين.

### الـsecrets الجديدة (بدل SMTP_USER/SMTP_PASS)
- `BREVO_API_KEY` — من Brevo Dashboard → SMTP & API → API Keys
- `BREVO_SENDER_EMAIL` — الإيميل اللي وثّقته كـ"Sender" في Brevo
  (لازم يكون Verified وإلا الإرسال هيترفض)
- `BREVO_SENDER_NAME` — اختياري، افتراضيًا "سَنَد"

`sendCodeEmail()` بقت بتنده على `https://api.brevo.com/v3/smtp/email`
بدل الاتصال بـSMTP. مفيش أي اتصال TCP/SMTP خام في الكود دلوقتي —
كل الاتصالات الخارجية عبر HTTPS عادي، بنفس نمط باقي إيدج فانكشنز
المشروع (telegram-send، ai-chat، إلخ).

توازن الأقواس اتفحص بعد التعديل — متطابق (إيدج فانكشن: 205/205،
109/109، 7/7 — الشاشة: 164/164، 75/75، 26/26).
